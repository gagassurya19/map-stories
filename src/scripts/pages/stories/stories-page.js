...
export default class StoriesPage {
  async render() {
    ...
    return `
      <section style="display: grid; grid-template-columns: 300px 1fr; height: 100vh;">
        <!-- Stories sidebar -->
        <div style="border-right: 1px solid #dee2e6; overflow: hidden; position: relative;">
          <div class="p-2 overflow-y-auto h-full">
            <div class="flex items-center justify-between mb-2">
              <h1 class="text-lg font-medium">Stories</h1>
              <div class="flex items-center gap-2">
                <a href="#/add-guest-story" class="inline-flex items-center gap-1 px-2 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700">
                  <i class="bi bi-plus-lg"></i>
                  <span>New as Guest</span>
                </a>
                <a href="#/add-story" class="inline-flex items-center gap-1 px-2 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700">
                  <i class="bi bi-plus-lg"></i>
                  <span>New</span>
                </a>
              </div>
            </div>
            <div id="storiesList" class="space-y-2 pb-12">
              <!-- Loading indicator -->
          ...
          <div class="fixed bottom-4 left-4 right-4 max-w-[272px]">
            ...
            >
              <i class="bi bi-arrow-down-circle-fill mr-1"></i> Load More
            </button>
          </div>
        </div>
        ...
        <div style="position: relative;">
          ...
          <div id="storiesMap" style="height: 100%; width: 100%;"></div>
        </div>
      </section>
    `;
  }
  ...
  initMap() {
    ...
    this.map = L.map('storiesMap', {
      minZoom: 4, // Minimum zoom level (Indonesia level)
      maxZoom: 18 // Maximum zoom level
    }).setView([-2.5489, 118.0149], 4); // Center on Indonesia
  ...
  displayStories() {
    ...
    storiesList.innerHTML = this.stories.map(story => `
      <div class="story-card bg-white rounded shadow-sm hover:shadow transition-shadow">
        <div class="relative">
          <img src="${story.photoUrl}" class="w-full h-32 object-cover rounded-t" alt="${story.name}'s story">
          ${story.lat && story.lon ? `
            <div class="absolute bottom-1 right-1">
              <span class="inline-flex items-center px-1.5 py-0.5 bg-blue-600 text-white text-xs rounded-full">
                <i class="bi bi-geo-alt-fill mr-0.5"></i> ${story.lat}, ${story.lon}
              </span>
            </div>
          ` : ''}
        </div>
        <div class="p-2">
          ...
          <div class="flex items-center justify-between gap-2 mt-1">
            <a href="#/stories/${story.id}" class="inline-block text-xs text-blue-600 hover:text-blue-800">
            ...
            ${story.lat && story.lon ? `
              <button 
                ...
                data-lon="${story.lon}"
              >
                <i class="bi bi-map mr-1"></i> View on map
              </button>
            ` : ''}
          </div>
        </div>
